...
namespace HelpfulThings.Connect.Cryptowatch
{
    ...
    public class MarketsClient : IMarketsClient
    {
        ...
        {
            return await _router.MakeRequest<List<MarketSummary>>(CryptowatchEndpoints.AllMarkets);
        }

        public async Task<Market> GetMarketDetailAsync(string exchange, string pair)
        {
            var formatedRoute = string.Format(CryptowatchEndpoints.MarketDetail, exchange, pair);
            return await _router.MakeRequest<Market>(formatedRoute);
        }

        public async Task<Price> GetPriceAsync(string exchange, string pair)
        {
            var formatedRoute = string.Format(CryptowatchEndpoints.Price, exchange, pair);
        ...
        {
            var formatedRoute = string.Format(CryptowatchEndpoints.Summary, exhcange, pair);
        ...
        {
            var formatedRoute = string.Format(CryptowatchEndpoints.OrderBook, exhcange, pair);
        ...
        public async Task<OhlcCollection> GetOhlcAsync(string exhcange, string pair, long after = 0, long before = 0,
            List<TimeBarLength> periods = null)
        {
            var formatedRoute = string.Format(CryptowatchEndpoints.Ohlc, exhcange, pair);
